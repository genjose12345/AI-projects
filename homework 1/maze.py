...
class MazeProblem(Problem):
    ...
    def actions(self, state):
        ...
        # YOUR CODE HERE..................................................................................
        maze = state.maze
        # removes up from the possible_actions List if it is a #
        if maze.at((x-1, y)) == "#":
            possible_actions.remove("UP")
        # removes down from the possible_actions list if it is a #
        if maze.at((x + 1, y)) == "#":
            possible_actions.remove("DOWN")
        # removes left from the possible_actions list if it is a #
        if maze.at((x, y-1)) == "#":
            possible_actions.remove("LEFT")
        # removes right from the possible_actions list if it is a #
        if maze.at((x, y+1)) == "#":
            possible_actions.remove("RIGHT")
        # removes TELEPORT from the possible_actions list if it is a #
        if maze.at((state.position)) != "o":
    ...
    def result(self, state, action):
        ...
        #change the current position if you move up by 1
        if action == "UP":
            new_position = (state.position[0] - 1, state.position[1])
        #changes the current position if you can move down by 1
        elif  action == "DOWN":
            new_position = (state.position[0] + 1, state.position[1])
        #changes the current position if you can move left by 1
        elif  action == "LEFT":
            new_position = (state.position[0], state.position[1] - 1)
        #changes the current postions if you can move right by 1
        elif  action == "RIGHT":
            new_position = (state.position[0], state.position[1] + 1)
        #changes postions based on the teleporter selcete first and the position of the one you are going to
        elif  action == "TELEPORT" and len(maze.teleporters) > 0:
            if state.position == state.maze.teleporters[0]:
                new_position = state.maze.teleporters[1]

            elif state.position == state.maze.teleporters[1]:
